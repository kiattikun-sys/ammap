...
import type { ListEvidenceFilter } from "./list-evidence";
...
export async function listEvidenceServer(
  ...
  if (error) throw new Error(`listEvidenceServer: ${error.message}`);
  return (data ?? []).map((r: Record<string, unknown>) => rowToEvidence(r));
}
