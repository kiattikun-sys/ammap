...
import type { ListTimelineEventsFilter } from "./list-timeline-events";
...
export async function listTimelineEventsServer(
  ...
  if (error) throw new Error(`listTimelineEventsServer: ${error.message}`);
  return (data ?? []).map((r: Record<string, unknown>) => rowToTimelineEvent(r));
}
