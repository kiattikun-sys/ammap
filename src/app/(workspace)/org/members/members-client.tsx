...
export function MembersClient({
  ...
  function handleAdd(e: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await addOrgMemberByEmail(addEmail, addRole);
      if (!result.ok) {
  ...
  function handleRoleChange(member: OrgMember, newRole: OrgRole) {
    setActionError(null);
    startTransition(async () => {
      const result = await updateOrgMemberRole(member.userId, newRole);
      if (!result.ok) {
  ...
  function handleRemove(member: OrgMember) {
    ...
    setActionError(null);
    startTransition(async () => {
      const result = await removeOrgMember(member.userId);
      if (!result.ok) {
        setActionError(result.error);
        return;
      }
    ...
    <div className="space-y-6">
      ...
      {canManage && (
        <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {addSuccess && <p className="mt-2 text-sm text-green-600">เพิ่มสมาชิกสำเร็จ</p>}
                ...
                <tr key={m.userId} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    {canEdit ? (
                      ...
                      <span
                        className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-semibold ${ROLE_BADGE[m.role]}`}
                      >
                        {ROLE_LABELS[m.role]}
                      </span>
                    )}
        ...
        <div className="grid grid-cols-2 gap-1 sm:grid-cols-4">
          {(["owner", "admin", "member", "viewer"] as OrgRole[]).map((r) => (
            <div key={r} className="flex items-center gap-1.5">
              <span
                className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${ROLE_BADGE[r]}`}
              >
                {ROLE_LABELS[r]}
              </span>
